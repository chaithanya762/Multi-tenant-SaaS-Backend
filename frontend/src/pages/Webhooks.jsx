...
export function Webhooks() {
  ...
  const handleCreate = async (e) => {
    e.preventDefault();
    try {
      const generatedSecret = `whsec_${Math.random().toString(36).substring(2)}${Date.now()}`;
      const payload = { ...newHook, secret: generatedSecret };
      const res = await apiFetch('/v1/webhooks', { method: 'POST', body: JSON.stringify(payload) });
